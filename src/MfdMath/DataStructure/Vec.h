 ...
 */
template<int dim, typename T> class Vec;
template<int dim, typename T> Vec<dim,T> operator * (T d, const Vec<dim,T>& v);

template<int dim, typename T>
class Vec
{
public:
	enum PARAMETER {m=dim};
	STATIC_ASSERT(m > 4);
	...
	{
		Vec result;
		for (int i = 0; i < m; i++) result[i] = x[i]+v.x[i];
		return result;
	}
	...
	{
		Vec result;
		for (int i = 0; i < m; i++) result[i] = x[i]+d;
		return result;
	}
...
public:
	...
	__host__ __device__ Vec<3,T>& operator = (const Vec<2,T_>& other)
	{
		x = (T)other.x;
		y = (T)other.y;
		z = 0;
	...
	template<int dim_, typename T_>
	__host__ __device__ operator Vec<dim_,T_>()
	{
	...
	__host__ __device__ inline T operator [] (unsigned int id) const
	{
		return (&x)[id];
	}

	__host__ __device__ inline T Dot(const Vec &v) const
	{
		return x*v.x + y*v.y + z*v.z;
	}

	__host__ __device__ inline Vec Cross(const Vec &v) const
	{
		return Vec(y * v.z - z * v.y, z * v.x - x * v.z , x * v.y - y * v.x);
	}

	__host__ __device__ inline T Normalize()
	{
		T l = Length();
		if (l < EPSILON)
		{
	...
	__host__ __device__ inline const Vec operator - (void) const
	{
		return Vec(-x, -y, -z);
	}

	__host__ __device__ inline Vec operator + ( const Vec &v ) const
	{
	...
	__host__ __device__ inline Vec operator + ( T d ) const
	{
	...
	__host__ __device__ inline void operator += ( const Vec &v )
	{
		x += v.x; y += v.y; z += v.z;
	}
	...
	__host__ __device__ inline void operator += ( T d )
	{
		x += d; y += d; z += d;
	}
	...
	__host__ __device__ friend Vec<3,T> operator * (T d, const Vec<3,T>& v)
	{
		return v * d;
	}
};
