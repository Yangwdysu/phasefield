...
{
public:
	enum PARAMETER {m=dim_m,n=dim_n,size=dim_m*dim_n};
	T x[m*n+(m*n==0)];

	Matrix() { for(int i=0;i<size;i++) x[i]= (T)0.0; }
	Matrix( T a ) { for(int i=0;i<size;i++) x[i]= a; }
	Matrix( const Matrix& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	...
	{
		Matrix mat;
		for(int i=0;i<size;i++) mat.x[i] = x[i]+A.x[i];
		return mat;
	}
...
public:
	...
	MatrixSq() : Matrix() {};
	MatrixSq( T a ) : Matrix(a) {};
	MatrixSq( const MatrixSq& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	MatrixSq( const Matrix<dim, dim, T>& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
...
public:
	...
	MatrixSq( T a ) : Matrix(a) {};
	MatrixSq( const MatrixSq& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	MatrixSq( const Matrix<4, 4, T>& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }

	MatrixSq( float m0, float m4, float  m8, float m12,
			  float m1, float m5, float  m9, float m13,
			  float m2, float m6, float m10, float m14,
			  float m3, float m7, float m11, float m15 )
	{
		x[0]=m0; x[4]=m4; x[8] =m8;  x[12]=m12;
		x[1]=m1; x[5]=m5; x[9] =m9;  x[13]=m13;
		x[2]=m2; x[6]=m6; x[10]=m10; x[14]=m14;
		x[3]=m3; x[7]=m7; x[11]=m11; x[15]=m15;
	}
...
public:
	...
	MatrixSq( T a ) : Matrix(a) {};
	MatrixSq( const MatrixSq& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	MatrixSq( const Matrix<3, 3, T>& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }

	MatrixSq( float m0, float m1, float  m2,
			  float m3, float m4, float  m5,
			  float m6, float m7, float  m8)
	{
		x[0]=m0; x[1]=m1; x[2]=m2; 
		x[3]=m3; x[4]=m4; x[5]=m5;
		x[6]=m6; x[7]=m7; x[8]=m8;
	}
...
public:
	Rotation3D() : MatrixSq() {};
	Rotation3D( T a ) : MatrixSq(a) {};
	Rotation3D( const Matrix<3, 3, T>& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	Rotation3D( const Rotation3D& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }

	Rotation3D( float m0, float m3, float m6,
...
{
public:
	Transform3D() : MatrixSq() {};
	Transform3D( T a ) : MatrixSq(a) {};
	Transform3D( const Matrix<4, 4, T>& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	Transform3D( const Transform3D& A ) { for(int i=0;i<size;i++) x[i] = A.x[i]; }
	
	Transform3D( float m0, float m4, float  m8, float m12,
				 float m1, float m5, float  m9, float m13,
				 float m2, float m6, float m10, float m14,
				 float m3, float m7, float m11, float m15 )
