...
#define mf 0.000001
	...
	// 	std::cout << "Center: " << lc.x << " " << lc.y << std::endl;
		...
		*/
		...
		m_trail->animate(dt, m_phasefield->m_cuda_position, m_phasefield->m_cuda_Velu, m_phasefield->m_cuda_Velv,m_phasefield->m_cuda_Velw);
		//m_phasefield->moveDynamicRegion(new_x - originX, new_z - originZ, m_phasefield->m_cuda_Velv, m_trail->m_device_grid, m_trail->m_grid_pitch);
		m_trail->resetSource();
		m_phasefield->animate(0.0005, m_trail->m_device_grid, m_trail->m_grid_pitch, m_ocean_patch->getDisplacement());//for pahsefield
		//synchronCheck;
	...
	void Coupling::initialize(RigidBody * boat, PhaseField* wave)
	{
		...
		int nx = center.x / dg - 512 / 2;//getGridSize()=512
		int ny = center.z / dg - 512 / 2;
