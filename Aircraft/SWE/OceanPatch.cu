...
// 
OceanPatch::OceanPatch(int size, float patchSize, int windType)
{
...
void OceanPatch::initialize()
{
	cufftPlan2d(&fftPlan, m_size, m_size, CUFFT_C2C);

	int spectrumSize = m_spectrumW*m_spectrumH * sizeof(float2);
	cudaCheck(cudaMalloc((void **)&m_h0, spectrumSize));
	//synchronCheck;
	float2* host_h0 = (float2 *)malloc(spectrumSize);
	generateH0(host_h0);

	cudaCheck(cudaMemcpy(m_h0, host_h0, spectrumSize, cudaMemcpyHostToDevice));
	...
	cudaCheck(cudaMalloc((void **)&m_gradient, m_size*m_size * sizeof(float4)));
...
void OceanPatch::animate(float t)
{
	...
	cufftExecC2C(fftPlan, m_Dzt, m_Dzt, CUFFT_INVERSE);
 	//cudaCheck(cudaGraphicsMapResources(1, &m_cuda_displacement_texture));
 	//cudaArray* cuda_displacement_array = nullptr;
 	//cudaArray_t cuda_displacement_array = nullptr;
 	//cudaCheck(cudaGraphicsSubResourceGetMappedArray(&cuda_displacement_array, m_cuda_displacement_texture, 0, 0));
 
 	//cudaCheck(cudaGraphicsMapResources(1, &m_cuda_gradient_texture));
 	//cudaArray* cuda_gradient_array = nullptr;
 	//cudaCheck(cudaGraphicsSubResourceGetMappedArray(&cuda_gradient_array, m_cuda_gradient_texture, 0, 0));
	...
	synchronCheck;
 	//cudaCheck(cudaMemcpyToArray(cuda_displacement_array, 0, 0, m_displacement, m_size*m_size * sizeof(float4), cudaMemcpyDeviceToDevice));
 	//cudaCheck(cudaMemcpyToArray(cuda_gradient_array, 0, 0, m_gradient, m_size*m_size * sizeof(float4), cudaMemcpyDeviceToDevice));

 	//cudaCheck(cudaGraphicsUnmapResources(1, &m_cuda_displacement_texture));
 	//cudaCheck(cudaGraphicsUnmapResources(1, &m_cuda_gradient_texture));
}
