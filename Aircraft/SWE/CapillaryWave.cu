...
#define max(a,b) (((a) > (b)) ? (a) : (b))
	...
	//void CapillaryWave::getVelocity(gridpoint gp,float u)
	//{
	...
	__global__ void C_demo(Grid3f position, Grid1f vel_u, Grid1f vel_v, Grid1f vel_w, gridpoint* grid_next, size_t pitch)
	{
		int i = threadIdx.x + blockIdx.x * blockDim.x;
		int j = threadIdx.y + blockIdx.y * blockDim.y;
		...
		float u1;
		float v;
		if (i >= 1 && i < 50 - 1 && j >= 1 && j < 60 - 1 && k >= 1 && k < 50 - 1)
		{
			...
			{
				//printf("%f", position(i, j, k).y);
				vel_v(i, j, k) = 0.0f;
				velu += 0.5*(vel_u(i, j, k) + vel_u(i + 1, j, k));
				velw += 0.5*(vel_w(i, j, k) + vel_w(i, j, k + 1));
				gp.x = velu*h;
				gp.y = velw*h;

				grid2Dwrite(grid_next, gx, gy, pitch, gp);
	...
	void CapillaryWave::animate(float dt, Grid3f position, Grid1f vel_u,Grid1f vel_v, Grid1f vel_w)
	{
		int extNx = m_simulatedRegionWidth + 2;
		int extNy = m_simulatedRegionHeight + 2;
		...
		for (int iter = 0; iter < nStep; iter++)
		{
			...
			//C_demo << < blocksPerGrid1, threadsPerBlock1 >> > (position, vel_u, vel_v, vel_w, m_device_grid_next, m_grid_pitch);
			swapDeviceGrid();
			synchronCheck;
		}
